...
class Method():
    ...
    def get_way(self, url, params=None, headers=None, cookies=None):
        logging.info('发起get请求')
        res = requests.get(url=url, params=params, headers=headers, cookies=cookies, verify=False)
        return res

    def post_way(self, url, data=None, headers=None, cookies=None):
        logging.info('发起post请求')
        res = requests.post(url=url, data=data, headers=headers, cookies=cookies, verify=False)
        return res

    def main_way(self, method, url, data=None, headers=None, cookies=None):
        if method == 'get':
            res = self.get_way(url, data, headers, cookies)

        else:
            res = self.post_way(url, data, headers, cookies)
...
if __name__ == '__main__':
    ...
    requests_way = 'post'
    header = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:68.0) Gecko/20100101 Firefox/68.0', 'Accept-Encoding': 'gzip, deflate, br',
              'Connection': 'keep-alive', 'Referer': 'https://www.rzmwzc.com/', 'Accept-Language': 'zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2'}
